...
export default class MonacoGitDiffView {
  ...
  constructor(editor, editorElement) {
    ...
    // HACK: This is NOT how atom is achieving this!
    this.subscriptions.add(
      editor.onDidSave(({path}) => {
        if (this.repository) {
          // This forces repository to trigger change event
          // and let tree-view know about the change
          this.repository.getPathStatus(path);

          // but it doesn't work with github stage view
          const github = atom.packages.getActivePackage('github');
          if (github) {
            github.mainModule.getActiveRepository().refresh();
          }
        }
      })
    );
  ...
  async subscribeToRepository() {
    ...
    const updateDiffs = this.updateDiffs.bind(this);
    this.repository = await this.repositoryForPath(this.editorPath);
    if (this.repository !== null) {

      this._repoSubs = new CompositeDisposable(
        this.repository.onDidChangeStatuses(updateDiffs),
        this.repository.onDidChangeStatus(changedPath => {
          if (changedPath === this.editorPath) updateDiffs();
        }),
        this.editor.onDidStopChanging(updateDiffs),
