...
export default class MonacoEditorView {
  ...
  constructor(options = {}) {
    ...
    // TODO: Is there a way to change these whithout destroying the editor?
    const renderWhitespace = atom.config.get('editor.showInvisibles') ? "boundary" : "none";
    const lineNumbers = atom.config.get('editor.showLineNumbers') ? "on" : "off";
    const autoIndent = atom.config.get('editor.autoIndent') ? "advanced" : "none";
    const renderIndentGuides = atom.config.get('editor.showIndentGuide');
    const wordWrap = atom.config.get('editor.softWrap') ? "on" : "off";
